...
https://github.com/dodo-saba/fit2gpx/tree/main/src
"""
...
class Converter:
    ...
    def __init__(self, status_msg: bool = False):
        ...
        # (use the same name as the field names in FIT file to facilitate parsing)
    ...
    def fit_to_gpx(self, f_in, f_out):
        ...
        for field in enhanced_fields:
            if (
                df_points[field].count() == 0
                and df_points[f"enhanced_{field}"].count() > 0
            ):
...
class StravaConverter(Converter):
    ...
    def __init__(self, dir_in, dir_out=None):
        ...
        if "activities.csv" not in os.listdir(dir_in) or not os.path.isdir(
            dir_in + "activities"
        ):
            raise Exception(
                "The input directory must be the main Strava data download directory, "
    ...
    def unzip_activities(self):
        ...
        zip_paths = [
            self._dir_activities + f
            for f in os.listdir(self._dir_activities)
            if ".gz" in f
        ]
    ...
    def strava_fit_to_gpx(self):
        ...
        for f_activity in fit_files:
            ...
            df_laps, df_points = self.fit_to_dataframes(
                self._dir_activities + f_activity
            )

            # Step 3.2: Identify FIT file metadata from Strava log
            # -- identify corresponding activity in md
            md = (
                df_acts.loc[df_acts["Filename"].str.contains(f_activity)]
                .iloc[0, :]
                .to_dict()
            )
            ...
            if self.status_msg:
                print(
                    f"{len(fit_files)} files have been converted from .fit to .gpx files containing Strava metadata"
                )
    ...
    def add_metadata_to_gpx(self):
        ...
        gpx_files = [
            f
            for f in os.listdir(self._dir_activities)
            if ".gpx" in f and f not in os.listdir(self._dir_out)
        ]
